    ...
    for page in range(1, total_pages + 1):
        ...
        for item in property_items:
            try:
                ...
                # Get district from <h6>.
                h6_tag = item.find("h6")
                district = h6_tag.text.strip() if h6_tag else ""

                # Get price from the <h5> with class "price" and remove non-digit characters.
                price_tag = item.find("h5", class_="price")
                price = price_tag.text.strip() if price_tag else ""
                price = re.sub(r'[^\d]', '', price)  # now price is just a number string

                # Get image URL from the property-image div style property.
                image_url = ""
                ...
                # Extract features: beds, baths, internal m2, external m2.
                beds = None
                baths = None
                int_m2 = None
                ext_m2 = None
                ...
                print("Scraped:", name, "(ID:", web_id, ") - Beds:", beds, ", Baths:", baths, ", INT:", int_m2, ", EXT:", ext_m2, " Price:", price)
...
def save_to_database(properties):
    ...
    # Create the sales_properties table.
    cur.execute("""
    ...
    # Create the sales_prices table with a UNIQUE constraint on web_id so that each property has only one price row.
    cur.execute("""
    ...
    # Build a set of web_ids for properties in the current scrape.
    scraped_web_ids = {prop["web_id"] for prop in properties if prop["web_id"] is not None}

    for prop in properties:
        web_id = prop["web_id"]
        # Upsert the property into sales_properties.
        cur.execute("SELECT id FROM sales_properties WHERE web_id = %s", (web_id,))
        ...
        # Upsert the price row in sales_prices.
        cur.execute("SELECT id FROM sales_prices WHERE web_id = %s", (web_id,))
        cur.execute("""
    ...
    # Disable any properties in sales_properties not found in this scrape.
    if scraped_web_ids:
        cur.execute(
            "UPDATE sales_properties SET enabled = FALSE WHERE web_id NOT IN %s",
            (tuple(scraped_web_ids),)
        )
...
if __name__ == "__main__":
    print("Starting sales scraping process...")
    properties = scrape_sales()
    print("Scraping completed. Updating database...")
    save_to_database(properties)
    print("Sales script finished successfully.")
